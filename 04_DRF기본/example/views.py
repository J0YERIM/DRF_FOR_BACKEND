...
@api_view(['GET', 'POST']) # GET/POST 요청을 처리하게 만들어줒는 데코레이터
def booksAPI(request): # /book/
    if request.method == 'GET': # GET 요청(도서 전체 정보)
        ...
        # Book.objects.all()을 통해 전체 데이터를 가져오려 하였으니, books에 들어온 데이터는 1개 이상의 Book 모델 데이터임
        # 따라서 시리얼라이저에 넣을 때 many=True 옵션에 넣어 여러 데이터에 대한 처리를 하도록 함
        ...
        # 요청으로 들어온 데이터를 역직렬화하여 모델에 집어넣어야 하므로 먼저 시리얼라이저에 request.data를 넣음
...
class BooksAPIMixins(mixins.ListModelMixin, mixins.CreateModelMixin, generics.GenericAPIView):
    queryset = Book.objects.all() # queryset에는 모델에 질문을 보내 받아온 데이터가 들어가는데 일반적으로 그냥 모든 데이터를 불러옴
    serializer_class = BookSerializer # serializer_class는 해당 API에서 사용할 시리얼라이저를 설정
...
class BookAPIMixins(mixins.RetrieveModelMixin, mixins.UpdateModelMixin, mixins.DestroyModelMixin, generics.GenericAPIView):
    ...
    lookup_field = 'bid' # Django 기본 모델 pk가 아닌 bid를 pk로 사용하고 있으니 lookup_field로 설정함
